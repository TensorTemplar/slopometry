...
class ExperimentOrchestrator:
    ...
    def analyze_commit_chain(self, base_commit: str, head_commit: str) -> None:
        ...
        for i, commit_sha in enumerate(commits):
            ...
            try:
                with self.git_tracker.extract_files_from_commit_ctx(commit_sha) as temp_dir:
                    if not temp_dir:
                        continue
                    ...
                    if previous_metrics:
                        ...
                        type_hint_color = (
                            "green" if type_hint_change > 0 else "red" if type_hint_change < 0 else "yellow"
                        )
                        delta_table.add_row(
                            "Type Hint Coverage",
                        ...
                        docstring_color = (
                            "green" if docstring_change > 0 else "red" if docstring_change < 0 else "yellow"
                        )
                        delta_table.add_row(
                            "Docstring Coverage",
                ...
                console.print(f"[yellow]Skipping commit {commit_sha[:8]}: {e}[/yellow]")
