...
OPENCODE_STORAGE = Path(os.environ["OPENCODE_STORAGE"]) if os.environ.get("OPENCODE_STORAGE") else None


@pytest.fixture
def real_opencode_session() -> tuple[str, Path]:
    """Pick the first OpenCode session that has a message directory, or skip."""
    if OPENCODE_STORAGE is None or not OPENCODE_STORAGE.is_dir():
        pytest.skip("OPENCODE_STORAGE env var not set or path does not exist")
...
class TestRealOpenCodeSessionExtraction:
    def test_extract_memories_from_opencode_session__produces_non_empty_conversation_from_real_session(
        self, real_opencode_session: tuple[str, Path]
    ):
    ...
    def test_extract_memories_from_opencode_session__contains_tool_markers_when_session_uses_tools(
        self, real_opencode_session: tuple[str, Path]
    ):
        session_id, storage_root = real_opencode_session
        extractor = MemoryExtractor("https://llm.example/v1", "model-x", "test-key")
...
class TestEndToEndFreshnessPipeline:
    ...
    def test_validate_freshness__supersede_links_old_to_new_via_superseded_by(self, fresh_memory_service: MemoryService):
        ...
        assert decisions[0].action == FreshnessAction.SUPERSEDE
        ...
        all_memories = fresh_memory_service.get_memories(project_dir="/test/proj", limit=100, include_superseded=True)
    ...
    def test_validate_freshness__merge_action_rewrites_candidate_content(self, fresh_memory_service: MemoryService):
        existing = MemoryEntry(
            id="old-2",
        ...
        assert decisions[0].action == FreshnessAction.MERGE
    ...
    def test_validate_freshness__dedupe_action_skips_new_save(self, fresh_memory_service: MemoryService):
        existing = MemoryEntry(
            id="old-3",
        ...
        assert len(decisions) == 1
        assert decisions[0].action == FreshnessAction.DEDUPE

        deduped_candidates: list[MemoryCandidate] = []
        for d in decisions:
            if d.action == FreshnessAction.DEDUPE:
    ...
    def test_validate_freshness__keep_both_saves_both_independently(self, fresh_memory_service: MemoryService):
        existing = MemoryEntry(
            id="old-4",
