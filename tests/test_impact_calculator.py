...
NOTE: Baseline comparison uses TOTAL metrics (sums, not averages).
"""
...
def test_calculate_impact__returns_neutral_when_deltas_match_baseline_average(calculator, repo_baseline):
    """When deltas match baseline mean exactly, z-scores are zero and impact is neutral."""
    delta = ComplexityDelta(
        total_complexity_change=2,  # Matches mean (which is 2.0 in the fixture)
        avg_complexity_change=0.1,
        total_volume_change=100.0,
        avg_volume_change=5.0,
        avg_difficulty_change=0.5,
        total_effort_change=50.0,  # Matches mean
        avg_effort_change=2.5,
        total_mi_change=-0.5,  # Matches mean
        avg_mi_change=-0.025,
        net_files_change=0,
    )
...
def test_calculate_impact__returns_improvement_when_all_metrics_better_than_average(calculator, repo_baseline):
    """When all metrics are better than average, should return positive impact score."""
    delta = ComplexityDelta(
        total_complexity_change=-4,  # Below average (good, mean is 2)
        avg_complexity_change=-0.2,
        total_volume_change=-100.0,
        avg_volume_change=-5.0,
        avg_difficulty_change=-0.5,
        total_effort_change=-25.0,  # Below average (good, mean is 50)
        avg_effort_change=-1.25,
        total_mi_change=2.0,  # Above average (good, higher MI is better, mean is -0.5)
        avg_mi_change=0.1,
        net_files_change=0,
    )
...
def test_calculate_impact__returns_degradation_when_all_metrics_worse_than_average(calculator, repo_baseline):
    """When all metrics are worse than average, should return negative impact score."""
    delta = ComplexityDelta(
        total_complexity_change=10,  # Above average (bad, mean is 2.0)
        avg_complexity_change=0.5,
        total_volume_change=500.0,
        avg_volume_change=25.0,
        avg_difficulty_change=2.5,
        total_effort_change=150.0,  # Above average (bad, mean is 50)
        avg_effort_change=7.5,
        total_mi_change=-2.0,  # Below average (bad, lower MI is worse, mean is -0.5)
        avg_mi_change=-0.1,
        net_files_change=0,
    )
...
def test_safe_z_score__returns_zero_when_std_is_zero_and_value_equals_mean(calculator):
    """When std dev is zero and value equals mean, z-score is zero."""
    z = calculator._safe_z_score(5.0, 5.0, 0.0)
    assert z == 0.0


def test_safe_z_score__returns_clamped_value_when_std_is_zero_and_value_differs(calculator):
...
def test_categorize_impact__returns_minor_degradation_in_range(calculator):
    """Score between -1.0 and -0.5 is minor degradation."""
    assert calculator._categorize_impact(-0.75) == ImpactCategory.MINOR_DEGRADATION


def test_categorize_impact__returns_significant_degradation_below_threshold(calculator):
    """Score < -1.0 is significant degradation."""
    assert calculator._categorize_impact(-1.5) == ImpactCategory.SIGNIFICANT_DEGRADATION


def test_calculate_impact__preserves_raw_deltas_in_assessment(calculator, repo_baseline):
    """Assessment should contain the original raw total delta values."""
    delta = ComplexityDelta(
        total_complexity_change=7,
        avg_complexity_change=0.35,
        total_volume_change=200.0,
        avg_volume_change=10.0,
        avg_difficulty_change=1.0,
        total_effort_change=75.0,
        avg_effort_change=3.75,
        total_mi_change=-0.8,
        avg_mi_change=-0.04,
        net_files_change=2,
    )
