        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with db._get_db_connection() as conn:
                ...
                cache_manager.save_metrics_to_cache(
                    "session1", "/test/repo", "abc123", test_metrics, None, "hash1"
                )
    ...
    def test_clean_and_dirty_repo_coexistence(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with db._get_db_connection() as conn:
                ...
                cache_manager.save_metrics_to_cache(
                    "session1", "/test/repo", "abc123", clean_metrics, None, None
                )
                ...
                clean_cached, _ = cache_manager.get_cached_metrics(
                    "session1", "/test/repo", "abc123", None
                )
                
                # Retrieve dirty repo metrics  
                dirty_cached, _ = cache_manager.get_cached_metrics(
                    "session1", "/test/repo", "abc123", "dirtyhash"
                )
    ...
    def test_is_cache_valid_for_different_states(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with db._get_db_connection() as conn:
                ...
                cache_manager.save_metrics_to_cache(
                    "session1", "/test/repo", "abc123", test_metrics, None, None
                )
...
class TestWorkingTreeCachingIntegration:
    ...
    def test_session_complexity_metrics_dirty_repo_caching(
        self, mock_calc_hash, mock_uncommitted, mock_get_sha
    ):
